...
server = 'CONFITEC_NB123\SQLEXPRESS'
database = 'loja'
cnx = p.connect(
...
def gerar_nota_venda(id_venda):
    consulta = '''
        SELECT c.nome, c.cep, m.modelo, v.valor, v.data_venda
        FROM venda v
        INNER JOIN cliente_loja c ON c.id_cliente = v.id_cliente
        INNER JOIN motocicleta_loja m ON m.id_motocicleta = v.id_motocicleta
        WHERE v.id_venda = {}
    '''.format(id_venda)
...
'''.format(venda[0], venda[1], venda[2], venda[3], venda[4])

        nome_arquivo = 'nota_venda_{}.txt'.format(id_venda)
...
Giselle Santos, [29/06/2023 10:48]
atualizar = '''
            UPDATE cliente_loja
